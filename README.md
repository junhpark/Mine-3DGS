...
* **publish 는 원자적**: `.<name>.minegs-partial` 에 쓰고 성공 후 rename. 중단된 fusion 이 더 얇은
  surface 로 읽히지 않는다.
* **surface.json 은 정확도를 주장하지 않는다.** 어느 run·어느 dataset(`dataset_hash`)·어느 method 로
  만들었는지만 기록한다. claim 을 가능하게 할 뿐, 참으로 만들지 않는다.
* **geometry gate**: `--diagnostic` 이 아닌 `minegs eval geometry` 는 surface artifact 를 요구하고
  dataset identity·hash·frame·point file 존재를 확인한다. 원시 PLY 는 거부된다 —
...
| 1 Metric surface & evaluation | **1A metric surface artifact + depth fusion: implemented + structurally tested** — depth map → LOCAL_METRIC surface artifact, claim 을 담는 geometry 는 artifact 를 요구한다. **gsplat depth rendering (1B): NOT IMPLEMENTED**, **TSDF/mesh: NOT IMPLEMENTED**, 실제 GPU·실측 데이터 과학적 검증: **NOT VALIDATED** |
